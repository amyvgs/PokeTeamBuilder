  ...
  const storeTeam = () => {
    ...
    try {
      ...
      if (allTeams.length === 6) {
        setUiMessage(
          true,
          "You Already Have 6 Teams Stored. Delete one Before Adding Another"
        );
        return;
      }
  ...
  const updateTeam = () => {
    try {
      ...
      if (props.index === null || !props.name || !props.teamList) {
        setUiMessage(
          true,
          "All Proper Data Is Not Supplied To Complete Action"
        );
      }
    ...
    <div className="flex flex-col justify-end items-center w-3/5 table-lg:h-3/5 table-sm:h-full p-5 animate-fadeIn mt-20">
      ...
      {props.display ? (
        <div className="flex flex-row gap-5 w-full justify-center items-center mb-5">
          {!editName ? (
            <>
              <p className="font-PixelSans text-4xl text-white">
                {props.teamName}
              </p>
              <Edit
                sx={{ color: purple[400] }}
            ...
            <>
              <input
                className="w-full text-2xl p-2 text-white bg-transparent border-b focus:outline-none focus:ring-offset-0 focus:ring-0 focus:border-purple-400 focus:border-b-4 mb-5"
                type="text"
        ...
        <div className="flex flex-row items-center justify-center w-full gap-10 mb-5">
          <input
            className="w-full p-2 text-white bg-transparent border-b focus:outline-none focus:ring-offset-0 focus:ring-0 focus:border-purple-400 focus:border-b-4 mb-5"
            type="text"
      ...
      <div className="flex flex-row justify-center items-center space-x-4 w-full h-64">
        {props.display ? (
          <button
            className="bg-gray-800 rounded-xl p-2 w-60 hover:shadow-lg hover:shadow-purple-800 font-PixelSans text-white"
            onClick={updateTeam}
          >
            Update Team
          </button>
          ...
          <button
            className="bg-gray-800 h-full rounded-xl p-2 w-60 hover:shadow-lg hover:shadow-purple-800 font-PixelSans text-white"
            onClick={storeTeam}
          >
            Store Team
          </button>
        )}
        <div className="rounded-full p-2 text-red-500 hover:bg-slate-600 hover:cursor-pointer">
          <RestartAlt />
        </div>
      </div>
    </div>
