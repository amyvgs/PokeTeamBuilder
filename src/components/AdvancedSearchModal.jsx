...
export default function AdvancedSearchModal(props) {
    ...
    useEffect(() => {
        const updateResults = async () => {
            ...
            for (const item in requestedGeneration) {
                const res = await axios.get(`https://pokeapi.co/api/v2/generation/${requestedGeneration[item]}/`);
            ...
            for (const item in requestedColor) {
                const res = await axios.get(`https://pokeapi.co/api/v2/pokemon-color/${requestedColor[item]}/`);
            ...
            if (colorResults.length === 0 && genResults.length === 0 && typeResults.length === 0) {
                setResultList([]);
            }
            else {
    ...
    const accordionDisplay = [
        {
            "title": "Type",
            "content": ["Bug", "Dark", "Dragon", "Electric", "Fairy", "Fighting", "Fire", "Flying", "Ghost", "Grass", "Ground", "Ice", "Normal", "Poison", "Psychic", "Rock", "Steel", "Water"],
            "list": requestedTypes,
        ...
        {
            "title": "Color",
            "content": ["Red", "Blue", "Yellow", "Green", "Black", "Purple", "Gray", "White", "Pink"],
            "list": requestedColor,
    ...
    return (
        <div onClick={() => props.setShowModal(false)} className="fixed flex items-center justify-center inset-0 bg-black/60 z-50">
            <div onClick={(e) => e.stopPropagation()} className="flex flex-col bg-slate-700 h-1/2 w-1/2 opacity-100 p-5 overflow-y-scroll rounded-lg shadow-lg shadow-black text-white">
                <div className=" flex flex-row justify-between mb-5">
                    <h1 className="font-PixelSans text-4xl">Advanced Search</h1>
                    <p onClick={(e) => props.setShowModal(false)} className="text-2xl text-red-600 hover:font-bold hover:cursor-pointer hover:text-red-800">X</p>
                </div>

                <div className="mb-10 ml-5">
                    <h1 className="text-2xl">Search Characteristics</h1>
                    <div className="flex flex-col justify-center items-center mt-2">
                        {/* set up accordions for characteristics */}
                        {accordionDisplay.map((element) => {
                            return <Accordion title={element["title"]} content={element["content"]} list={element["list"]} updateList={element["updateList"]} />
                        })}
                    ...
                    {resultList.length === 0 ?
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            {resultList.map((element) => {
                                return (
                                    <div key={element.name} onClick={() => handlePokemonChoice(element.url)} className="bg-slate-800 col-span-1 p-2 rounded-lg shadow-md shadow-black hover:shadow-purple-600 hover:cursor-pointer">
