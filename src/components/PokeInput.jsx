  ...
  return (
    <div ref={inputContainerRef} className="relative">
      <input
        ...
        className="text-white bg-transparent border-b focus:outline-none focus:ring-offset-0 focus:ring-0 focus:border-purple-400 focus:border-b-10"
        type="text"
        placeholder="Choose a Pokemon"
        onChange={handleInputChange}
      />
      {showList && (
        <ul className="bg-slate-900 text-white h-28 w-full absolute overflow-scroll z-10">
          {filteredPokemon.map((pokemon, index) => (
            <li
              key={index}
              onClick={() => handlePokemonChoice(pokemon.url)}
              className="hover:bg-black hover:cursor-pointer p-2"
            >
